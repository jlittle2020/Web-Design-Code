<?php
   function dispatchCategories() {
	  ...
	  while ($row = mysqli_fetch_assoc($select)) {
         ...
         dispatchSubCategories($row['cat_id']);
		 echo "</table>";
      }
   }
   function dispatchSubCategories($parent_id) {
      include ('databaseconnect.php');
   ...
   function dispatchTopics($cid, $scid) {
      include ('databaseconnect.php');
      ...
      if (mysqli_num_rows($select) != 0) {
         ...
         while ($row = mysqli_fetch_assoc($select)) {
            echo "<tr><td><a href='readtopic.php?cid=".$cid."&scid=".$scid."&tid=".$row['topic_id']."'>
                 ".$row['title']."</a></td><td>".$row['author']."</td><td>".$row['date_posted']."</td><td>".$row['views']."</td></tr>";
         }
         echo "</table>";
      } else {
         echo "<p>This category does not have any topics yet! <a href='newtopic.php?cid=".$cid."&scid=".$scid."'>
   ...
   function replyLink($cid, $scid, $tid) {
      echo "<p><a href='reply.php?cid=".$cid."&scid=".$scid."&tid=".$tid."'>Reply to this topic</a></p>";
   }
   function replyToTopic($cid, $scid, $tid) {
   ...
   function displayReplies($cid, $scid, $tid) {
      include ('databaseconnect.php');
	  ...
	  $select = mysqli_query($con, "SELECT replies.author, comment, replies.date_posted FROM categories, subcategories,
                                    ...
                                    AND ($tid = replies.topic_id) AND ($cid = categories.cat_id) AND ($scid = subcategories.subcat_id)
                                    AND ($tid = topics.topic_id) ORDER BY reply_id DESC");
      if (mysqli_num_rows($select) != 0) {
